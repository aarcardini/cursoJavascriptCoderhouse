...
const inputConditions = { option1: 0};
...
const totalBackoverlayers = document.querySelector("#totalBackers");
...
const clearSelect = () => {
    ...
    if (currentSelection) {
        ...
        const investment = document.querySelector(".selection.active .investment");
        const currentInput = document.querySelector(".selection.active .investment input");
        currentSelection.classList.remove("active");
        radio.checked = false;
        investment.style.maxHeight = 0;
        setTimeout(() => {
...
const selectNew = select => {
    ...
    const investment = document.querySelector(".selection.active .investment");
    investment.style.maxHeight = investment.scrollHeight + "px";
    select.checked = true;
...
continueButtons.forEach(b => {
    b.addEventListener("click", event => {
        ...
        investment = Number(input.value);
        if (!investment || investment < inputConditions[inputID]) {
...
finalizeButton.addEventListener("click", () => {
    ...
    const newTotal = Math.round(parseFloat(totalRaised.innerHTML.replace(",", "")) + investment);
